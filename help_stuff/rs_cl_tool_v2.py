...
def large_func():
    dataset_files = os.listdir('../datasets/cl_datasets/high_imbalance')
    ...
    for d_fp in dataset_files:
            with open("../latex-gen/high_imbalance/" + d_fp + "-latex-tables.txt", "w") as latex_file:
                first_row = pd.read_csv('../datasets/cl_datasets/high_imbalance/' + d_fp, delimiter=',', nrows=1)
                header_row = has_header(first_row)
                tfr = pd.read_csv('../datasets/cl_datasets/high_imbalance/' + d_fp, delimiter=',', iterator=True, header=header_row)
...
def generate_latex_output(dict_with_data):
    ...
    for sampling_version in dict_with_data.keys():
        with open("../latex-gen/high_imbalance/" + sampling_version + "-latex-tables.txt", "w+", encoding="utf8") as latex_file:
            ...
            chapter_inited = False
        pre_table_inited = False
    with open("../latex-gen/high_imbalance/" + "br-latex-tables.txt", "w+", encoding="utf8") as br_latex_file:
